...
public class ImportLegacyDataUI implements Runnable {
    ...
    public void run() {
        ...
        String answer = String.format("Are you sure you want to import the legacy data file %s? (y/n)", filename);
        ...
        try {
            ...
            System.out.println("Error: the file content could not be parsed (it probably contains errors!).");
            e.printStackTrace();
        } catch (NotFoundException e) {
            System.out.printf("Error: %s not found.\n", e.getMessage());
            e.printStackTrace();
        } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | NoSuchMethodException | InvocationTargetException e) {
            System.out.println("Something wrong has happened. Here's something that might help:");
